...
function sumEvenArguments(){
    var argsArr = arrayFrom(arguments);
    var count = 0;
...
function bind(fn, thisArg){
    // BROAD IDEA IS TO COMBINE ALL THE ARGUMENTS INTO ONE BEFORE PASSING TO APPLY

    // return as an array all arguments except the first two passed into the function
...
function flip(fn, thisArg){
    var outerArgs = [].slice(arguments, 2);
    return function() {
        var innerArgs = [].slice(arguments);
        // fn.length returns the number of argument fn should include
        var allArgs = outerArgs.concat(innerArgs).slice(0, fn.length);
        return fn.apply(this, allArgs.reverse());
    }
}